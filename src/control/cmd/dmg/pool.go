...
func (cmd *PoolCreateCmd) Execute(args []string) error {
	...
	calcRatio := func(scm, nvme uint64) float64 {
		if nvme == 0 {
			return 1.00
		}
		return float64(scm) / float64(nvme)
	}
	...
	if cmd.Size != "" {
		...
		scmBytes := uint64(float64(req.TotalBytes) * req.ScmRatio)
		nvmeBytes := req.TotalBytes - scmBytes
		cmd.log.Infof("Creating DAOS pool with automatic storage allocation: "+
			"%s SCM, %s NVMe (%0.2f%% ratio)", humanize.Bytes(scmBytes),
			humanize.Bytes(nvmeBytes), req.ScmRatio*100)
	} else {
		...
		ratio := calcRatio(req.ScmBytes, req.NvmeBytes)
	...
	cmd.log.Info(txtfmt.FormatEntity(title, []txtfmt.TableRow{
		{"UUID": resp.UUID},
		{"Replica Ranks": formatRanks(resp.SvcReps)},
		{"Target Ranks": formatRanks(resp.TgtRanks)},
		{"Size": humanize.Bytes(resp.ScmBytes + resp.NvmeBytes)},
		{"SCM": fmt.Sprintf("%s (%s / rank)", humanize.Bytes(resp.ScmBytes), humanize.Bytes(resp.ScmBytes/uint64(len(resp.TgtRanks))))},
		{"NVMe": fmt.Sprintf("%s (%s / rank)", humanize.Bytes(resp.NvmeBytes), humanize.Bytes(resp.NvmeBytes/uint64(len(resp.TgtRanks))))},
	}))
